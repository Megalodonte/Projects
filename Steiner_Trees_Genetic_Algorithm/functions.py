...
def prim_alg(vertices):
    ...
    return(connections)                                       # the process is repeated until there are not anymore elements in remaining_points
    ...
    for g in range(NGEN):
        ...
        for mutant in offspring:
            if random.random() < MUTPB:
                toolbox.mutate(mutant, indpb=0.9)
...
def steiner_alg(n_fixed_points, n_steiner_point, my_sigma, POP_SIZE, CXPB, MUTPB, seed = None):
    ...
    #fixed_vertices = [[0.1,0.1],[0.4,0.1],[0.25,0.36]]
    if seed:    
        random.seed(seed)
...
def test_for_steiner_number (n_fixed_points, my_sigma, POP_SIZE, CXPB, MUTPB, seed = None):
    ...
    print("Difference between After and Before = ", best_istance - distance_before)
